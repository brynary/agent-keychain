...
The project keychain password is generated by the tool. When macOS allows the local binary to create user-presence-protected login-keychain items, the password is stored in the user's login keychain with `userPresence` access control.

Unsigned or ad-hoc CLI builds can receive `errSecMissingEntitlement` when creating that access-control-protected item. In that case, `agent-keychain` falls back to a normal login-keychain item and enforces user presence in-process before reading it. Reading the login-keychain item unlocks the project keychain for the operation, and the project keychain is locked again afterward.
